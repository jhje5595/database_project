...
DROP TABLE IF EXISTS `외부캠핑카 정비소`;
...
-- -----------------------------------------------------
-- Table `DBTEST`.`캠핑카 대여회사`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `DBTEST`.`캠핑카 대여회사` (
  `캠핑카 대여회사 ID` VARCHAR(40) NOT NULL,
  `회사명` VARCHAR(45) NULL,
  `주소` VARCHAR(45) NULL,
  `전화번호` VARCHAR(45) NULL,
  `담당자 이름` VARCHAR(10) NULL,
  `담당자 이메일` VARCHAR(45) NULL,
  PRIMARY KEY (`캠핑카 대여회사 ID`))
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `DBTEST`.`캠핑카` (
  `캠핑카 등록 ID` VARCHAR(40) NOT NULL,
  `캠핑카 이름` VARCHAR(45) NULL,
  `차량번호` VARCHAR(45) NULL,
  `승차인원수` INT NULL,
  `캠핑카 이미지` VARCHAR(255) NULL,
  `캠핑카 상세정보` VARCHAR(100) NULL,
  `캠핑카 대여비용` INT NULL,
  `대여한 회사 ID` VARCHAR(40) NULL,
  `등록일자` DATE NULL,
  PRIMARY KEY (`캠핑카 등록 ID`),
  INDEX `캠핑카 대여회사 아이디 외래키_idx` (`대여한 회사 ID` ASC) VISIBLE,
  CONSTRAINT `캠핑카대여회사id_외래키`
    FOREIGN KEY (`대여한 회사 ID`)
    REFERENCES `DBTEST`.`캠핑카 대여회사` (`캠핑카 대여회사 ID`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `DBTEST`.`직원` (
  `직원 ID` VARCHAR(40) NOT NULL,
  `직원 이름` VARCHAR(10) NULL,
  `휴대폰 번호` VARCHAR(20) NULL,
  `집주소` VARCHAR(45) NULL,
  `월급여` INT NULL,
  `부양가족수` INT NULL,
  `부서명` VARCHAR(45) NULL,
...
CREATE TABLE IF NOT EXISTS `DBTEST`.`자체정비기록` (
  `자체정비등록 ID` VARCHAR(40) NOT NULL,
  `캠핑카 등록 ID` VARCHAR(40) NULL,
  `부품등록 ID` VARCHAR(40) NULL,
  `정비일자` DATE NULL,
  `정비시간` INT NULL,
  `정비담당자 ID` VARCHAR(40) NULL,
  PRIMARY KEY (`자체정비등록 ID`),
  INDEX `캠핑카 등록id 외래키_idx` (`캠핑카 등록 ID` ASC) VISIBLE,
  INDEX `부품등록 id 외래키_idx` (`부품등록 ID` ASC) VISIBLE,
  INDEX `정비담당자 id 외래키_idx` (`정비담당자 ID` ASC) VISIBLE,
  CONSTRAINT `캠핑카 등록id 외래키`
    FOREIGN KEY (`캠핑카 등록 ID`)
    REFERENCES `DBTEST`.`캠핑카` (`캠핑카 등록 ID`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `부품등록 id 외래키`
    FOREIGN KEY (`부품등록 ID`)
    REFERENCES `DBTEST`.`부품재고` (`부품등록 ID`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `정비담당자 id 외래키`
    FOREIGN KEY (`정비담당자 ID`)
    REFERENCES `DBTEST`.`직원` (`직원 ID`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `DBTEST`.`고객` (
  `고객 ID` VARCHAR(40) NOT NULL,
  `비밀번호` VARCHAR(30) NULL,
  `운전면허증 번호` VARCHAR(40) NULL,
  `고객명` VARCHAR(10) NULL,
  `고객주소` VARCHAR(45) NULL,
  `고객 전화번호` VARCHAR(20) NULL,
  `고객 이메일` VARCHAR(45) NULL,
  `이전 캠핑카 사용날짜` DATE NULL,
  `이전 캠핑카 종류` VARCHAR(45) NULL,
  PRIMARY KEY (`고객 ID`),
  UNIQUE INDEX `운전면허증 번호_UNIQUE` (`운전면허증 번호` ASC) VISIBLE)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `DBTEST`.`캠핑카대여` (
  `대여번호` INT NOT NULL,
  `캠핑카 등록 ID` VARCHAR(40) NULL,
  `운전 면허증 번호` VARCHAR(40) NULL,
  `캠핑카 대여회사 ID` VARCHAR(40) NULL,
  `대여시작일` DATE NULL,
  `대여기간` INT NULL,
  `청구요금` INT NULL,
  `납입기한` DATE NULL,
  ...
  `기타청구요금` INT NULL,
  PRIMARY KEY (`대여번호`),
  INDEX `캠핑카 등록 id 외래키_idx` (`캠핑카 등록 ID` ASC) VISIBLE,
  INDEX `캠핑카 대여회사 id 외래키_idx` (`캠핑카 대여회사 ID` ASC) VISIBLE,
  INDEX `운전면허증번호 외래키_idx` (`운전 면허증 번호` ASC) VISIBLE,
  UNIQUE INDEX `운전 면허증 번호_UNIQUE` (`운전 면허증 번호` ASC) VISIBLE,
  CONSTRAINT `캠핑카 등록 id 외래키`
    FOREIGN KEY (`캠핑카 등록 ID`)
    REFERENCES `DBTEST`.`캠핑카` (`캠핑카 등록 ID`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `운전면허증번호 외래키`
    FOREIGN KEY (`운전 면허증 번호`)
    REFERENCES `DBTEST`.`고객` (`운전면허증 번호`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `캠핑카 대여회사 id 외래키`
    FOREIGN KEY (`캠핑카 대여회사 ID`)
    REFERENCES `DBTEST`.`캠핑카 대여회사` (`캠핑카 대여회사 ID`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
-- -----------------------------------------------------
-- Table `DBTEST`.`외부캠핑카 정비소`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `DBTEST`.`외부캠핑카 정비소` (
  `캠핑카정비소 ID` VARCHAR(40) NOT NULL,
  `정비소명` VARCHAR(45) NULL,
  `정비소 주소` VARCHAR(45) NULL,
  `정비소 전화번호` VARCHAR(20) NULL,
  `담당자 이름` VARCHAR(10) NULL,
  `담당자 이메일` VARCHAR(45) NULL,
  PRIMARY KEY (`캠핑카정비소 ID`))
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `DBTEST`.`외부정비정보` (
  ...
  `캠핑카 등록 ID` VARCHAR(40) NULL,
  `캠핑카 대여회사 ID` VARCHAR(40) NULL,
  `캠핑카정비소 ID` VARCHAR(40) NULL,
  `고객 운전면허증 번호` VARCHAR(40) NULL,
  ...
  PRIMARY KEY (`정비번호`),
  INDEX `캠핑카등록 id 외래키_idx` (`캠핑카 등록 ID` ASC) VISIBLE,
  INDEX `캠핑카 대여회사 id 외래키_idx` (`캠핑카 대여회사 ID` ASC) VISIBLE,
  INDEX `고객운전면허증 정보 외래키_idx` (`고객 운전면허증 번호` ASC) VISIBLE,
  INDEX `외부캠핑카 정비소 외래키_idx` (`캠핑카정비소 ID` ASC) VISIBLE,
  UNIQUE INDEX `고객 운전면허증 번호_UNIQUE` (`고객 운전면허증 번호` ASC) VISIBLE,
  CONSTRAINT `캠핑카등록id_외래키`
    FOREIGN KEY (`캠핑카 등록 ID`)
    REFERENCES `DBTEST`.`캠핑카` (`캠핑카 등록 ID`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `캠핑카대여회사id_정비_외래키`
    FOREIGN KEY (`캠핑카 대여회사 ID`)
    REFERENCES `DBTEST`.`캠핑카 대여회사` (`캠핑카 대여회사 ID`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `고객운전면허증 정보 외래키`
    FOREIGN KEY (`고객 운전면허증 번호`)
    REFERENCES `DBTEST`.`고객` (`운전면허증 번호`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `외부캠핑카 정비소 외래키`
    FOREIGN KEY (`캠핑카정비소 ID`)
    REFERENCES `DBTEST`.`외부캠핑카 정비소` (`캠핑카정비소 ID`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
-- 캠핑카 대여회사
INSERT INTO `캠핑카 대여회사` VALUES 
('CC001', '캠핑월드', '서울 강남구', '010-1111-1111', '홍길동', 'hong@example.com'),
...
(1, 'RV001', 'DL1234567', 'CC001', '2024-06-01', 3, 450000, '2024-06-04', '침낭 추가', 30000),
(2, 'RV002', 'DL2345678', 'CC002', '2024-06-05', 5, 600000, '2024-06-10', '자전거 대여', 50000);

-- 외부캠핑카 정비소
INSERT INTO `외부캠핑카 정비소` VALUES
